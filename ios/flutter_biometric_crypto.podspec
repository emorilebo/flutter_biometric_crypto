#
# To learn more about a Podspec see http://guides.cocoapods.org/syntax/podspec.html.
# Run `pod lib lint flutter_biometric_crypto.podspec` to validate before publishing.
#
Pod::Spec.new do |s|
  s.name             = 'flutter_biometric_crypto'
  s.version          = '0.1.1'
  s.summary          = 'A Flutter plugin for biometric-protected encryption'
  s.description      = <<-DESC
A Flutter package for biometric-protected encryption of small secrets using Android Keystore and iOS Keychain/Secure Enclave.
                       DESC
  s.homepage         = 'https://github.com/emorilebo/flutter_biometric_crypto'
  s.license          = { :file => '../LICENSE' }
  s.author           = { 'Godfrey Lebo' => 'emorylebo@gmail.com' }
  s.source           = { :path => '.' }
  s.source_files = 'Classes/**/*'
  s.dependency 'Flutter'
  s.platform = :ios, '12.0'

  # Flutter.framework does not contain a i386 slice.
  s.pod_target_xcconfig = { 'DEFINES_MODULE' => 'YES', 'EXCLUDED_ARCHS[sdk=iphonesimulator*]' => 'i386' }
  s.swift_version = '5.0'
end

